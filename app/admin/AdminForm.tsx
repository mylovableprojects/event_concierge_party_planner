...
export default function AdminForm({ config, initialInventory = [] }: { config: Config; initialInventory?: ParsedItem[] }) {
  ...
  // Current inventory editor
  const [currentInventory, setCurrentInventory] = useState<ParsedItem[]>(initialInventory)
  ...
  const updateItem = (i: number, field: keyof ParsedItem, value: string | number) =>
    setCurrentInventory(prev => prev.map((item, idx) => idx === i ? { ...item, [field]: value } : item))

  const removeItem = (i: number) =>
    setCurrentInventory(prev => prev.filter((_, idx) => idx !== i))
  ...
  const saveCurrentInventory = async () => {
    ...
    try {
      const res = await fetch('/api/inventory', {
        ...
        body: JSON.stringify({ items: currentInventory }),
      })
      ...
      setInventoryError(err instanceof Error ? err.message : 'Something went wrong')
    } finally {
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-8">
          ...
          {currentInventory.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="font-semibold text-gray-800">Current Inventory</h2>
                  <p className="text-xs text-gray-500 mt-0.5">{currentInventory.length} items — edit names, prices, or categories inline.</p>
                </div>
                <button
                  type="button"
                  onClick={saveCurrentInventory}
                  disabled={inventorySaving}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gray-900 text-white text-sm font-semibold hover:bg-gray-800 disabled:opacity-50 transition-colors"
                >
                  {inventorySaving ? 'Saving...' : inventorySaved ? '✓ Saved' : 'Save Inventory'}
                </button>
              </div>
              {inventoryError && (
                <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-700">{inventoryError}</div>
              )}
              <div className="space-y-1.5 max-h-[480px] overflow-y-auto pr-1">
                {currentInventory.map((item, i) => (
                  <div key={item.id} className="flex items-center gap-2 bg-white border border-gray-100 rounded-xl px-3 py-2.5">
                    <div className="flex-1 min-w-0">
                      <input
                        value={item.name}
                        onChange={e => updateItem(i, 'name', e.target.value)}
                        className="w-full text-sm font-medium text-gray-800 bg-transparent border-0 outline-none focus:bg-gray-50 rounded px-1 -mx-1"
                      />
                      <input
                        value={item.category}
                        onChange={e => updateItem(i, 'category', e.target.value)}
                        className="w-full text-xs text-gray-400 bg-transparent border-0 outline-none focus:bg-gray-50 rounded px-1 -mx-1 mt-0.5"
                      />
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <input
                        ...
                        className="w-20 text-sm font-bold text-gray-700 bg-transparent border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500 text-right"
                      />
                    </div>
                    <button
                      type="button"
          ...
          <div className="space-y-3">
            <div>
              <h2 className="font-semibold text-gray-800">
                {currentInventory.length > 0 ? 'Replace Inventory' : 'Inventory'} <span className="text-gray-400 font-normal text-sm">(optional — skip to update settings only)</span>
              </h2>
